...
通俗的来说，作者可以找一个自己喜欢作品的文档，把该作品使用本项目向量化后，使用自然语言描述自己的想法和需求，让本项目去在范本（也就是提供的作品）里找符合自然语言描述的文段。
也就是当作者写不来了之后可以找别人的片段学一下怎么写，而不是让ai直接给生成ai语料和建议。
还有一个是主要功能是给长篇小说用的知识库以及剧情索引，该用途来自于项目作者懒癌发作，连自己描述想法都不想做了，让ai找薄弱点，然后自动和范本对比优化，但是担心ai理解不了上下文，于是顺带做了一个完整的长文本管理。
